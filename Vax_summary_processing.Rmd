...
vax3 <- vax2 %>%
  ...
  mutate(across(c(survey_date, onset_date, archive_firstvaxdate,vax_date, vaxbeforedate, vaxafterdate ), as.Date),
         ...
         across(c(survey_date, onset_date, archive_firstvaxdate,vax_date, vaxbeforedate, vaxafterdate), as.Date)) %>%

  # consolidate all vaccines
  mutate(vaxbeforedate = ifelse(is.na(vaxbeforedate) & before_vax == 1, survey_date, vaxbeforedate),
         vaxafterdate = ifelse(is.na(vaxafterdate) & after_vax == 1, onset_date-1, vaxafterdate),
         vax_date = ifelse(!is.na(vax_number) & vax_number >= 1, survey_date, vax_date),
         across(c(survey_date, onset_date, archive_firstvaxdate,vax_date, vaxbeforedate, vaxafterdate), as.Date)) %>%
  rowwise() %>%
  mutate(       
...
```{r}
# Flags and admin variables are only kept in redcap event 000_arm_1, so they are pulled separately to filter the main dataset.
vaccines <- redcap_read_oneshot(
  redcap_uri = RedCapUrl,
  token = Token2024,
  guess_type = FALSE,
  fields =  c('record_id','firstvaxdate','firstvaxdatetype'),
  filter = "[000_arm_1][covhort_survey_complete] = '2' and [000_arm_1][remove(1)]<>1 and [000_arm_1][remove(2)]<>1 and [000_arm_1][dup]<>1 and [000_arm_1][possiblespam]<>1",
  events = '000_arm_1',
...
```{r}
vax_update <- vax3 %>%
  left_join(vaccines %>% rename(firstvaxdateo = firstvaxdate, firstvaxdatetypeo = firstvaxdatetype), by = "record_id") %>%
  filter(firstvaxdateo != firstvaxdate) %>%
  select(record_id, firstvaxdate, firstvaxdatetype)
```
